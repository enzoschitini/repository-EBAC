...
lista = ['SINASC_RO_2019', 'SINASC_RO_2019_1osemestre']

for periodo in lista:
    filename = r'{current_directory}\input\{periodo}.csv'.format(current_directory=current_directory, periodo=periodo)
    sinasc = pd.read_csv(filename)

    max_data = sinasc.DTNASC.max()[:7]
    max_data = f'{max_data}_out'
    # print(max_data)
